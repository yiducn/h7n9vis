d3 = function() {
  ...
  function d3_interpolateNumber(a, b) {
    b -= a = +a;
    return function(t) {
//    	console.log("uninterpolate:"+a+":"+b+":"+t+":"+(a + b * t));
      return a + b * t;
    };
  }
  ...
  function d3_uninterpolateNumber(a, b) {
    b = b - (a = +a) ? 1 / (b - a) : 0;
    return function(x) {
//    	console.log("uninterpolate:"+a+":"+b+":"+x+":"+((x - a) * b));
      return (x - a) * b;
    };
  }
  ...
  function d3_interpolate2(a, b) {
	 
	  var m=(a+b)/2;
	  var k = (b-a)/(b*b-2*b*m+m*m+a*a-2*a*m+m*m);
	  var n = a + k*(a-m)*(a-m);
	  console.log("m:"+m+":k:"+k+":n"+n);
	    return function(t) {
	    	if(t >= (a+b)/2){
	    		console.log("interpolate:"+a+":"+b+":"+t+":"+(k*(t-m)*(t-m)+n));
	    		return k*(t-m)*(t-m)+n;
	    	}else{
	    		console.log("interpolate:"+a+":"+b+":"+t+":"+(-1*k*(t-m)*(t-m)+n));
	    		return -1*k*(t-m)*(t-m)+n;
	    	}
//	      return a + b * t;
	    };
	  }
  d3.scale.linear2 = function() {
	    return d3_scale_linear2([ 0, 1 ], [ 0, 1 ], d3_interpolate2, false);
	  };
	  function d3_scale_linear2(domain, range, interpolate, clamp) {
	    var output, input;
	    //此时，domain是一个数值数组，range是个范围(像素值)
	    function rescale() {
	      var linear = Math.min(domain.length, range.length) > 2 ? d3_scale_polylinear : d3_scale_bilinear, uninterpolate = clamp ? d3_uninterpolateClamp : d3_uninterpolateNumber;
	      output = linear(domain, range, uninterpolate, interpolate);
	      input = linear(range, domain, uninterpolate, d3_interpolate);
	    ...
	    function d3_uninterpolateNumber2(a, b) {
//	    	console.log("ab:"+a+":"+b);
//	    	var c = b;
//	    	b = a;
//	    	a = c;
//	    	console.log("ab:"+a+":"+b);
		  ...
		  var n = a + k*(a-m)*(a-m);
		    return function(t) {
		    	if(t >= (a+b)/2){
		    		console.log("uninterpolate:"+a+":"+b+":"+t+":"+(Math.sqrt(Math.abs((t-n)/k))+m));
		    		return (m+Math.sqrt(Math.abs((t-n)/k)));
		    	}else{
		    		console.log("uninterpolate:"+a+":"+b+":"+t+":"+(m- Math.sqrt(Math.abs((t-n)/k))));
		    		return (m-Math.sqrt(Math.abs((t-n)/k)));
		    	}
		    };
	      }

	    function d3_scale_polylinear2(domain, range, uninterpolate, interpolate) {
	        var u = [], i = [], j = 0, k = Math.min(domain.length, range.length) - 1;
	        console.log("d3_scale_polylinear2:domainlength:"+range+":"+domain);
	        while (++j <= k) {
	          u.push(uninterpolate(domain[j - 1], domain[j]));
	          i.push(interpolate(range[j - 1], range[j]));
//	          console.log("rrr:"+range);
	        }
	        return function(x) {
	        	//二分查找
	          var j = d3.bisect(domain, x, 1, k) - 1;
...
//	          var temp2 = 1+ (temp-1)*(Math.abs(range[0]-range[1])/99);
//	          console.log("temp2:"+temp2);
	    ...
	    function scale(x) {
	    	...
	    	output = function(x){//默认在轴中间进行focus
	    		var t = 285;//中间的像素值
	    		var temp = 70;//focus 的值 domain
	    		var v = ((t*x)/temp)*((t*x)/temp);	    		
	    		if(x>temp){	    			
	    			x = Math.abs(100-x);
	    			v = ((t*x)/temp)*((t*x)/temp);
	    			return t-Math.sqrt(Math.abs(t*t - v));
	    		}
	    		else{
	    			return t+Math.sqrt(Math.abs(t*t - v));
	    		}
...
//	      console.log("call range:"+range);
	    ...
	    scale.ticks = function(m) {
//	    	console.log("call ticks"+m);
	      return d3_scale_linearTicks2(domain, m);
	    };
	    function d3_scale_linearTicks2(domain, m) {
	        return d3.range.apply(d3, d3_scale_linearTickRange2(domain, m));
	      }
	    function d3_scale_linearTickRange2(domain, m) {
	        if (m == null) m = 10;
	        var extent = d3_scaleExtent2(domain);
...
//	        console.log(extent+":extent");
	    ...
	    function d3_scaleExtent2(domain) {
	        var start = domain[0], stop = domain[domain.length - 1];
//	        console.log("scale extent:"+start+":"+stop);
	        return start < stop ? [ start, stop ] : [ stop, start ];
	      }
...
//	      return ticks;
	    ...
	    scale.tickFormat = function(m, format) {
//	    	console.log("call tickFormat"+domain);
	      return d3_scale_linearTickFormat(domain, m, format);
	    };
//	    scale.ticks = function(m) {
//	      return d3_scale_linearTicks(domain, m);
...
//    console.log(extent+":extent");
  ...
  function d3_scale_ordinal(domain, ranger) {
    ...
    //自定义的range points
    //focuseIndex第几个出于focus
    //startStep开始的大小
    scale.rangePointsFc = function(x, padding, focusedIndex, startStep) {
    	console.log(":"+x+":"+focusedIndex+":"+padding+":"+domain.length);
    	
        var start = x[0], stop = x[1], step = (stop - start) / (Math.max(1, domain.length - 1) + padding);
        var locationFocusedIndex = start+step*padding/2+focusedIndex*step;
        console.log("location:"+locationFocusedIndex);
        var d1 = ((((locationFocusedIndex-step*padding/2)*2)/focusedIndex)	-startStep*2)/(focusedIndex-1);//pos
        var d2 = ((((stop-padding*step/2-locationFocusedIndex)*2)/(domain.length-1-focusedIndex))  - startStep*2) /(domain.length-1-focusedIndex-1);
console.log("d1:"+d1);//neg
        console.log("d2:"+d2);
        
       
        range[0] = start+step*padding/2;
        temp = range[0];
        var i;
        for(i = 1; i < focusedIndex; i ++){
        	range[i] = range[i-1] +(startStep+(i-1)*d1);
        	temp = range[i];
        	console.log(temp);
        }
        range[focusedIndex] = locationFocusedIndex;
        temp = range[focusedIndex];
        i ++;
        console.log("hello");
        for(i = focusedIndex+1; i < domain.length; i ++){
        	console.log(d2);
        	range[i] = range[i-1] +(startStep+(domain.length-i-1)*d2);
        	temp = range[i];
        	console.log(temp);
        }
        console.log(range);
        //range = [];
        	//steps(domain.length < 2 ? (start + stop) / 2 : start + step * padding / 2, step);
        rangeBand = 0;
        ranger = {
          t: "rangePoints",
          a: arguments
  ...
  var d3_time_days = [ "星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六" ], d3_time_dayAbbreviations = [ "日", "一", "二", "三", "四", "五", "六" ], d3_time_months = ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月","十二月"], d3_time_monthAbbreviations = [ "一", "二","三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二" ];
