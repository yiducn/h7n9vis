...
pc.autoscale = function() {
  ...
  var defaultScales = {
    ...
    "number": function(k) {   
    	if(k == "latitude"){
    		// d3.axis.tickFormat(function(d){return d.toString()+"C";});
    ...
    .rangePoints([h()+1, 1]);				
    		...
    		return  d3.scale.ordinal()
    .domain(["Unknown","Others","Livestock industry","Poultry industry","No job","Children","Student", "Farmer","Leader","Worker","Framer worker","Free Worker","Retirement",
    	"Cook", "Driver", "Teacher", "Doctor","Biddy","Taiwan businessman","Individual household" ])
    .rangePoints([h()+1, 1]);	
    
    	}
    	if(k == "status"){
    		return  d3.scale.ordinal()
    .domain(["Unknown", "Dead", "veryheavy","heavy", "light", "good"])
    .rangePoints([h()+1, 1]);				
    	}
    	else{
      return d3.scale.ordinal()
...
pc.createAxes = function() {
  ...
  g.append("svg:g")
      ...
      .each(function(d) {
      	// if(d == "sex"){
      		// d3.select(this).call(
      			// // d3.svg.axis().scale(x).tickSize(-height).tickSubdivide(true)
      			// d3.svg.axis().scale(
      				// d3.scale.ordinal()
    // .domain(["男", "未知", "女"])
    // .rangePoints([0, __.height])
...
//创建图例 duyi 
pc.createLegend = function() {
  var legend = pc.svgParent.append("svg:g").attr("class", "legend")
  .attr("transform", "translate(" + __.margin.left + "," + __.margin.top + ")");
  if(__.legend.length == 0){
  	console.log("no legend data");
    return this;
  }    
  
  var i = 0;
  for(var p in __.legend){
  	legend.append("svg:rect")
  	.attr("x", 10+i*80)
  	.attr("y", __.height-30-30)
  	.attr("height", 20)
  	.attr("width", 40)
  	.style("fill", __.legend[p])
  	.attr("transform", "translate(" + __.margin.left + "," + __.margin.top + ")");
  	legend.append("text")
  	.attr("width",40)
  .attr("x", 60+i*80)
